...
import { manicMiner, jetpac, zynaps, badWav } from 'data';

const audioFiles = [
  { label: 'Manic Miner', value: manicMiner },
  { label: 'Jetpac', value: jetpac },
  { label: 'Zynaps', value: zynaps },
  { label: 'bad.wav', value: badWav },
];

export const App = () => {
  const [src, setSrc] = useState(audioFiles[0].value);
        ...
        <nav>
          <select value={src} onChange={(event) => {
            ...
            {audioFiles.map((audioFile, index) => (
              <option key={index} value={audioFile.value}>{audioFile.label}</option>
            ))}
          </select>
